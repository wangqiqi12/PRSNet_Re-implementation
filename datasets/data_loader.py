...
def test_dataloader():
    """Test data loader"""
    print("Testing ShapeNet data loader...")
    ...
    dataloader = create_shapenet_dataloader(
        ...
        max_samples=16  # Limit samples for quick testing
    )
    ...
    for batch_idx, (voxel_batch, model_ids) in enumerate(dataloader):
        ...
        print(f"  Voxel data shape: {voxel_batch.shape}")  # Should be [4, 1, 32, 32, 32]
        print(f"  Data type: {voxel_batch.dtype}")
        print(f"  Value range: [{voxel_batch.min().item():.3f}, {voxel_batch.max().item():.3f}]")
        print(f"  Model IDs: {model_ids[:2]}...")  # Show first two IDs
        
        # Calculate occupancy rate for each sample
        occupancy_rates = []
        for i in range(voxel_batch.shape[0]):
            sample = voxel_batch[i, 0]  # Remove channel dimension
            occupancy = (sample > 0.5).sum().item() / sample.numel()
            occupancy_rates.append(occupancy)
        
        print(f"  Occupancy rates: {[f'{rate*100:.1f}%' for rate in occupancy_rates]}")
...
def test_with_prsnet():
    """Test data loader integration with PRSNet"""
    print("\nTesting integration with PRSNet model...")
    ...
    print(f"PRSNet parameter count: {sum(p.numel() for p in model.parameters()):,}")
    
    # Test forward pass
    with torch.no_grad():
        for batch_idx, (voxel_batch, model_ids) in enumerate(dataloader):
            print(f"\nBatch {batch_idx}:")
            print(f"  Input shape: {voxel_batch.shape}")
            
            # Forward pass
            outputs = model(voxel_batch)
            
            print(f"  Output:")
            print(f"    planes shape: {outputs['planes'].shape}")  # [batch_size, 3, 4]
            print(f"    quats shape: {outputs['quats'].shape}")    # [batch_size, 3, 4]
            
            # Check output value ranges
            planes = outputs['planes']
            quats = outputs['quats']
            
            print(f"    planes range: [{planes.min().item():.3f}, {planes.max().item():.3f}]")
            print(f"    quats range: [{quats.min().item():.3f}, {quats.max().item():.3f}]")
            
            # Verify quaternion normalization
            quat_norms = torch.norm(quats, dim=-1)
            print(f"    quaternion norms: [{quat_norms.min().item():.3f}, {quat_norms.max().item():.3f}]")
            
            # Verify plane normal normalization
            plane_normals = planes[:, :, :3]
            normal_norms = torch.norm(plane_normals, dim=-1)
            print(f"    plane normal norms: [{normal_norms.min().item():.3f}, {normal_norms.max().item():.3f}]")
            
            if batch_idx >= 1:  # Only test first 2 batches
                break
    
    print("\nPRSNet integration test completed!")
